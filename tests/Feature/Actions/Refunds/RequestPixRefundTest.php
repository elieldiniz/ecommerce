...
use App\Actions\Refunds\RequestPixRefund;
...
class RequestPixRefundTest extends TestCase
{
    use RefreshDatabase;
    ...
    private function authorizedPayment(): Payment
    {
        ...
        return Payment::factory()->create([
            'status_id' => $status->id,
            'gateway_transaction_id' => 'TXN-PIX-1',
        ]);
    }
    ...
    public function test_a_payment_that_is_not_authorized_is_rejected_without_creating_a_refund(): void
    {
        ...
        try {
            (new RequestPixRefund)->execute($payment, $reason, $user, '127.0.0.1');
    ...
    public function test_a_valid_request_creates_the_refund_immediately_but_payment_status_only_changes_via_webhook(): void
    {
        ...
        $refund = (new RequestPixRefund)->execute($payment, $reason, $user, '127.0.0.1');
        ...
        Http::assertSent(fn ($request) => $request->method() === 'DELETE'
            && $request->url() === 'https://payment.safe2pay.com.br/v2/payment/TXN-PIX-1/cobranca_pix-estornar');
    }
    ...
    public function test_a_failed_sync_call_creates_exactly_one_additional_audit_log_referencing_the_refund(): void
    {
        ...
        $failureLog = AuditLog::query()->where('action', 'pix_refund_sync_call_failed')->first();
