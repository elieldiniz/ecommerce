...
new #[Layout('components.admin-layout', ['activeItem' => 'formas-pagamento', 'title' => 'Formas de pagamento'])] #[Title('Editar cupom')] class extends Component
{
    public int $id;
    ...
    public function updateCoupon(): void
    {
        ...
        DB::transaction(function () use ($validated): void {
            Coupon::findOrFail($this->id)->update($validated);
        });
    ...
    {
        return [
            'code' => ['required', 'string', 'max:40', Rule::unique('coupons', 'code')->ignore($this->id)],
...
<div>
    <div class="flex items-center justify-between">
        <h1 class="font-heading text-xl font-bold text-ink">Editar cupom</h1>
    ...
    <section class="mt-6 rounded-xl border border-border bg-white p-5">
        <form wire:submit="updateCoupon" class="grid grid-cols-1 gap-4 md:grid-cols-2">
            <div>
            ...
            <div class="md:col-span-2">
                <button type="submit" class="rounded-lg bg-brand px-4 py-2.5 font-heading text-xs font-semibold text-white">Salvar alterações</button>
            </div>
        </form>
    </section>
</div>
